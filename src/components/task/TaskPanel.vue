    ...
    <a-table class="plan-table" :columns="columns" :data-source="plansData.data" row-key="job_id"
      ...
      <template #action="{ record }">
        <div class="action-area">
          ...
          <a-popconfirm v-if="record.status === TaskStatus.Paused" title="Are you sure you want to resume?" ok-text="Yes"
            cancel-text="No" @confirm="onResumeTask(record.job_id)">
            <a-button type="primary" size="small">恢复</a-button>
          </a-popconfirm>

          <!-- <a-popconfirm v-if="record.status === TaskStatus.Carrying" title="确定返航吗?" ok-text="Yes" -->
          <a-popconfirm  title="确定返航吗?" ok-text="Yes"
            cancel-text="No" @confirm="onReturnHome(record.job_id)">
            <a-button type="primary" size="small">返航</a-button>
          </a-popconfirm>
...
async function onReturnHome (jobId: string) {
  const { code } = await updateTaskStatus(workspaceId, {
    job_id: jobId,
    status: UpdateTaskStatus.Resume
  })
